...
public class AvroSenderImpl implements Sender {
    ...
    public void send(String serverId, Map<String, ExecutionInfo> info) {
        ...
        for (String key : info.keySet()) {
            ...
            if (executionInfo.getDetails() != null) {
                for (String k : executionInfo.getDetails().keySet()) {
                    ...
                    details.put(k, new ExecutionInfoBean(executionInfo.getContextPath(), v.getExecutionCount(), v.getFailedCount(), v.getExecutionTimeTotal(), System.currentTimeMillis(), new HashMap<CharSequence, ExecutionInfoBean>()));
                }
            }

            ExecutionInfoBean bean = new ExecutionInfoBean(executionInfo.getContextPath(), executionInfo.getExecutionCount(), executionInfo.getFailedCount(), executionInfo.getExecutionTimeTotal(), System.currentTimeMillis(), details);
            result.put(key, bean);
        }
        try {
            if (result.size() > 0) {
                client.send(result, serverId);
            }
